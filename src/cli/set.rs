...
pub fn execute(manager: &mut SettingsManager, args: &[String]) -> Result<String, String> {
    let key = args
        .first()
        .ok_or_else(|| "usage: mitos-settings set <key> <value>".to_string())?;
    let raw_value = args
        .get(1)
        .ok_or_else(|| "usage: mitos-settings set <key> <value>".to_string())?;
    ...
    let value =
        Value::parse(kind, raw_value).map_err(|e| format!("invalid value for '{key}': {e}"))?;
    manager
        .set(key, value)
...
mod tests {
    ...
    fn sets_a_valid_value() {
        ...
        let result = execute(
            &mut manager,
            &["sound.volume".to_string(), "70".to_string()],
        );
        assert!(result.is_ok());
    ...
    fn rejects_invalid_value_before_touching_the_manager() {
        ...
        let result = execute(
            &mut manager,
            &["sound.volume".to_string(), "not-a-number".to_string()],
        );
    ...
    fn rejects_unknown_key() {
        ...
        let result = execute(
            &mut manager,
            &["nonexistent.key".to_string(), "1".to_string()],
        );
        assert!(result.is_err());
        std::fs::remove_dir_all(dir).ok();
    }
