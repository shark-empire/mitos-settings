...
fn changing_a_relevant_setting_regenerates_home_conf() {
    ...
    manager
        .set("appearance.accent_color", Value::Str("#00ff00".into()))
        .unwrap();
    manager
        .set("appearance.glass_opacity", Value::Float(0.5))
        .unwrap();
    manager
        .set("appearance.dock_enabled", Value::Bool(false))
        .unwrap();
...
fn manager_without_a_home_conf_path_never_writes_one() {
    ...
    let mut manager = SettingsManager::with_stores(
        Mode::Standalone,
        Store::at(dir.join("user.conf")),
        Store::at(dir.join("system.conf")),
    )
    ...
    manager
        .set("appearance.accent_color", Value::Str("#123456".into()))
        .unwrap();
...
fn invalid_hex_accent_color_is_rejected_before_it_ever_reaches_home_conf() {
    ...
    let err = manager
        .set("appearance.accent_color", Value::Str("not-a-color".into()))
        .unwrap_err();
